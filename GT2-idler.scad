...
motor_shaft = 15.2;	// NEMA17 motor shaft exact diameter = 5
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	// smaller collar for non flanged part of bearing, needs to be small enough to allow one 1mm washer to get squeezed in between
	translate([0, 0, (pulley_t_ht+pulley_b_ht+retainer_ht)/2]){
		difference() {
			cylinder(r=motor_shaft/2+0.2, h=6.0, center=true, $fn= 64);
			cylinder(r=13.2/2, h=6.0+0.2, center=true, $fn= 64);
		}
	 	// support layer for bottom indent
		#translate([0, 0, -6.0/2])  cylinder(r=motor_shaft/2+0.2, h=0.2, center=false);
	}
